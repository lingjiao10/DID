...
def match_3_terms(threshold, truths, priors, variances, labels, loc_t, conf_t, bin_conf_t, idx):
    """Match each prior box with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    Args:
        ...
        loc_t: (tensor) Tensor to be filled w/ endcoded location targets.
        conf_t: (tensor) Tensor to be filled w/ matched indices for conf preds.
        bin_conf_t: (tensor) Tensor to be filled w/ matched indices for bin_conf preds.
        idx: (int) current batch index
    Return:
        The matched indices corresponding to 1)location and 2)confidence preds.
    """
    ...
    loc_t[idx] = loc    # [num_priors,4] encoded offsets to learn
    conf_t[idx] = conf  # [num_priors] top class label for each prior
    bin_conf_t[idx] = bin_conf  # [num_priors] top class label for each prior, indicating weather an object or not


def encode(matched, priors, variances, idx):
